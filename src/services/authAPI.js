import apiClient from "./apiClient";

export const authAPI = {
  async getSignMessage(walletAddress) {
    const res = await apiClient.post("/auth/sign-message", { walletAddress });
    return res.data;
  },

  async login(walletAddress, message, signature) {
    const payload = { walletAddress, message, signature };
    const res = await apiClient.post("/auth/login", payload);
    // Backend returns { success, data: { token, profile }, message }
    return res.data?.data || res.data;
  },

  async verify() {
    const res = await apiClient.get("/auth/verify");
    return res.data;
  },
};

export const profileAPI = {
  async getProfile(walletAddress) {
    const res = await apiClient.get(`/profile/${walletAddress}`);
    // Backend returns { success, data: { ...profile } }
    return res.data?.data || res.data;
  },

  async updateNickname(walletAddress, nickname) {
    const res = await apiClient.patch(`/profile/${walletAddress}/nickname`, { nickName: nickname });
    // Backend returns { success, data: { ...profile }, message }
    return res.data?.data || res.data;
  },

  async uploadProfilePicture(walletAddress, formData) {
    const res = await apiClient.patch(`/profile/${walletAddress}/profile-picture`, formData, {
      headers: { "Content-Type": "multipart/form-data" },
    });
    // Backend returns { success, data: { ...profile }, message }
    return res.data?.data || res.data;
  },
};

export default authAPI;
